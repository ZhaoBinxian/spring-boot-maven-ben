 ...
 */
@TestPropertySource(
        properties = "spring.config.location=classpath:/org/springframework/boot/actuate/autoconfigure/endpoint/web" +
                "/documentation/")
    ...
    void env() throws Exception {
        this.mockMvc.perform(get("/actuator/env")).andExpect(status().isOk())
                .andDo(document("env/all", preprocessResponse(replacePattern(
                                Pattern.compile("org/springframework/boot/actuate/autoconfigure/endpoint/web" +
                                        "/documentation/"),
                                ""), filterProperties()),
