...
class ValidationFailureAnalyzerTests {
    ...
    void analyzesMissingRequiredConfiguration() {
        ...
        assertThat(analysis.getDescription()).isEqualTo(String.format("Invalid Micrometer configuration detected:%n%n"
                + "  - management.metrics.export.newrelic.apiKey was 'null' but it is required when publishing to " +
                "Insights API%n"
                + "  - management.metrics.export.newrelic.accountId was 'null' but it is required when publishing to " +
                "Insights API"));
    }
