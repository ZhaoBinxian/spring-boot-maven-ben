...
public class Neo4jHealthIndicator extends AbstractHealthIndicator {
     ...
     */
    static final String CYPHER = "CALL dbms.components() YIELD versions, name, edition WHERE name = 'Neo4j Kernel' " +
            "RETURN edition, versions[0] as version";

    /**
     * Message logged before retrying a health check.
     */
    static final String MESSAGE_SESSION_EXPIRED = "Neo4j session has expired, retrying one single time to retrieve " +
            "server health.";
