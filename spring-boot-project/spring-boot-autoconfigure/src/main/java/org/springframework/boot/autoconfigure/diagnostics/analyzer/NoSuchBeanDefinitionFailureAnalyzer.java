            ...
            if (configurationProperties.isPresent()) {
                if (KotlinDetector.isKotlinType(declaringClass) && !KotlinDetector.isKotlinReflectPresent()) {
                    action = String.format(
                            "%s%nConsider adding a dependency on kotlin-reflect so that the constructor used for @%s " +
                                    "can be located. Also, ensure that @%s is present on '%s' if you intended to use " +
                                    "constructor-based "
